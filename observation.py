...
from data_source import OptionDataSource, YFStockDataSource, YFCalendarDataSource
...
class DataManager:
    ...
    def get_options(self, ticker: str, query_date: date) -> pd.DataFrame:
        return self.option_source.get_chain(ticker, query_date)

    def get_stock_price(
        self, ticker: str, start: Optional[date] = None, end: Optional[date] = None
    ) -> pd.DataFrame:
        return self.stock_source.get_stock_price(ticker, start, end)

    def get_earnings(
        self, start: Optional[date] = None, end: Optional[date] = None
    ) -> pd.DataFrame:
        return self.calendar_source.get_earnings(start, end)
